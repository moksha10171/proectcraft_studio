...
import { ArrowLeft, ExternalLink, Brain, Download, Star, Clock, Users } from 'lucide-react';
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const resource = mlResources.resources.find((r: any) => r.slug === slug);
  ...
  return generatePageMetadata({
    title: `${resource.name} - Machine Learning Framework Guide`,
    description: resource.fullDescription || resource.shortDesc,
    path: `/resources/machine-learning/${slug}`,
    keywords: [resource.name, 'machine learning', 'AI', 'neural networks', 'deep learning'],
  });
}

export default async function MachineLearningResourcePage({ params }: PageProps) {
  const { slug } = await params;
  const resource = mlResources.resources.find((r: any) => r.slug === slug);
  ...
  const breadcrumbs = generateBreadcrumbs(`/resources/machine-learning/${slug}`);

  // Get theme color based on category
  const getThemeColor = (category: string) => {
    switch (category) {
      case 'Deep Learning Frameworks': return 'purple';
      case 'Traditional ML Libraries': return 'green';
      case 'NLP & Transformers': return 'blue';
      case 'MLOps & Production': return 'orange';
        ...
        <section className={`bg-gradient-to-b from-${themeColor}-500/5 via-background to-background py-12 md:py-20`}>
          <div className="container mx-auto px-6 sm:px-8 lg:px-12 xl:px-16">
            <Link
              ...
              aria-label="Go back to machine learning resources"
            >
              <ArrowLeft className="mr-2 h-4 w-4" />
              Back to Machine Learning Resources
            </Link>

            <div className="flex items-center gap-4 mb-6">
              <div className={`p-4 bg-${themeColor}-500/10 rounded-xl`}>
                <Brain className={`h-10 w-10 text-${themeColor}-500`} />
              </div>
              <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-2xl">
              ...
              <Card>
                <CardContent className="pt-4">
                  ...
                  <div className="text-lg font-semibold">AI/ML</div>
        ...
        <section className={`py-16 bg-${themeColor}-500/5`}>
          <div className="container mx-auto px-6 sm:px-8 lg:px-12 xl:px-16">
            <div className="max-w-3xl mx-auto text-center">
              <h2 className="text-3xl font-bold mb-4">Ready to Build AI with {resource.name}?</h2>
              <p className="text-muted-foreground mb-8 text-lg">
                Start your machine learning journey with hands-on projects and virtual environments. Build real AI applications.
              </p>
