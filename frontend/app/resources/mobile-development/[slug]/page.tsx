...
import { ArrowLeft, ExternalLink, Smartphone, Download, Star, Clock, Users } from 'lucide-react';
...
const typedResources = mobileResources as unknown as ResourceData;

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const resource = typedResources.resources.find((r) => r.slug === slug);
  ...
  return generatePageMetadata({
    title: `${resource.name} - Mobile Development Framework Guide`,
    description: resource.fullDescription || resource.shortDesc,
    path: `/resources/mobile-development/${slug}`,
    keywords: [resource.name, 'mobile development', 'iOS', 'Android', 'cross-platform', 'React Native', 'Flutter'],
  });
}

export default async function MobileDevelopmentResourcePage({ params }: PageProps) {
  const { slug } = await params;
  const resource = typedResources.resources.find((r) => r.slug === slug);
  ...
  const breadcrumbs = generateBreadcrumbs(`/resources/mobile-development/${slug}`);

  // Get theme color based on category
  const getThemeColor = (category: string) => {
    switch (category) {
      case 'Cross-Platform Frameworks': return 'indigo';
      case 'Native iOS Development': return 'blue';
      case 'Native Android Development': return 'green';
      case 'Hybrid Development': return 'purple';
      case 'Mobile UI/UX Design': return 'pink';
      case 'Development Tools': return 'orange';
    ...
    <>
      ...
      <main role="main" aria-label={`${resource.name} Details`} className="min-h-screen bg-background pb-20 md:pb-0">
        ...
        <section className={`bg-gradient-to-b from-${themeColor}-500/5 via-background to-background py-12 md:py-20`}>
          <div className="container mx-auto px-6 sm:px-8 lg:px-12 xl:px-16">
            <Link
              ...
              aria-label="Go back to mobile development resources"
            >
              <ArrowLeft className="mr-2 h-4 w-4" />
              Back to Mobile Development Resources
            </Link>

            <div className="flex items-center gap-4 mb-6">
              <div className={`p-4 bg-${themeColor}-500/10 rounded-xl`}>
                <Smartphone className={`h-10 w-10 text-${themeColor}-500`} />
              </div>
              <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-2xl">
              ...
              <Card>
                <CardContent className="pt-4">
                  ...
                  <p className="text-xs text-muted-foreground">of Apps</p>
                </CardContent>
              </Card>
            </div>
          </div>
          ...
          <section className="py-16 bg-muted/30">
            <div className="container mx-auto px-6 sm:px-8 lg:px-12 xl:px-16">
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {Object.entries(resource.features).map(([feature, description]) => (
        ...
        {resource.alternatives && resource.alternatives.length > 0 && (
          <section className="py-16">
            <div className="container mx-auto px-6 sm:px-8 lg:px-12 xl:px-16">
              <h2 className="text-3xl font-bold mb-8">Alternative Frameworks</h2>
        ...
        <section className={`py-16 bg-${themeColor}-500/5`}>
          <div className="container mx-auto px-6 sm:px-8 lg:px-12 xl:px-16">
            <div className="max-w-3xl mx-auto text-center">
              <h2 className="text-3xl font-bold mb-4">Ready to Build with {resource.name}?</h2>
              <p className="text-muted-foreground mb-8 text-lg">
                Start creating amazing mobile experiences. Build native apps that users love.
              </p>
