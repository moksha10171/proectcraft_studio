...
import { ArrowLeft, ExternalLink, BarChart3, Download, Star, Clock, Users } from 'lucide-react';
...
const typedDsResources = dsResources as unknown as ResourceData;
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const resource = typedDsResources.resources.find((r) => r.slug === slug);
  ...
  return generatePageMetadata({
    title: `${resource.name} - Data Science Tool Guide`,
    description: resource.fullDescription || resource.shortDesc,
    path: `/resources/data-science/${slug}`,
    keywords: [resource.name, 'data science', 'data analysis', 'visualization', 'statistics'],
  });
}

export default async function DataScienceResourcePage({ params }: PageProps) {
  const { slug } = await params;
  const resource = typedDsResources.resources.find((r) => r.slug === slug);
  ...
  const breadcrumbs = generateBreadcrumbs(`/resources/data-science/${slug}`);

  // Get theme color based on category
  const getThemeColor = (category: string) => {
    switch (category) {
      case 'Data Manipulation': return 'teal';
      case 'Data Visualization': return 'blue';
      case 'Statistical Computing': return 'green';
      case 'Interactive Computing': return 'purple';
      case 'Business Intelligence': return 'orange';
      case 'Big Data Processing': return 'indigo';
      default: return 'teal';
    }
  };
        ...
        <section className={`bg-gradient-to-b from-${themeColor}-500/5 via-background to-background py-12 md:py-20`}>
          <div className="container mx-auto px-6 sm:px-8 lg:px-12 xl:px-16">
            <Link
              ...
              aria-label="Go back to data science resources"
            >
              <ArrowLeft className="mr-2 h-4 w-4" />
              Back to Data Science Resources
            </Link>

            <div className="flex items-center gap-4 mb-6">
              <div className={`p-4 bg-${themeColor}-500/10 rounded-xl`}>
                <BarChart3 className={`h-10 w-10 text-${themeColor}-500`} />
              </div>
              <div>
          ...
          <div className="container mx-auto px-6 sm:px-8 lg:px-12 xl:px-16">
            <div className="max-w-3xl mx-auto text-center">
              ...
              <p className="text-muted-foreground mb-8 text-lg">
                Start your data science journey with hands-on projects and real datasets. Transform data into actionable insights.
              </p>
